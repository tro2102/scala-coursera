...
object FunSets {
  ...
  /**
    * Returns the set of the passed-in list
    */
  def setFromList(list: List[Int]): Set = (i) => list.contains(i)

  def sizeThreeList(start: Int): Set = setFromList(List(start, start+1, start+2))

  def consecutiveInts(start: Int, size: Int): Set =
    (i) => i >= start && i < start + size
